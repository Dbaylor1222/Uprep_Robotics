...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(Drive1_M) == (NULL));
  while (runModel) {
    mw_osSemaphoreWaitEver(&baserateTaskSem);
    Drive1_step();

    /* Get model outputs here */
    stopRequested = !((rtmGetErrorStatus(Drive1_M) == (NULL)));
  }
...
{
  UNUSED(sig);
  rtmSetErrorStatus(Drive1_M, "stopping the model");
  runModel = 0;
}
